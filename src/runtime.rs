...
fn run_codex(
    ...
    let mut child = command
        .spawn()
        .with_context(|| format!("Failed to spawn codex in {worktree:?}"))?;
    let pid = Some(child.id());
    let status = child
        .wait()
        .with_context(|| format!("Failed to wait for codex in {worktree:?}"))?;
...
fn run_claude(
    ...
    if background {
        ...
        let mut child = command
            .spawn()
            .with_context(|| format!("Failed to spawn claude in {worktree:?}"))?;
        let pid = Some(child.id());
        let status = child
            .wait()
            .with_context(|| format!("Failed to wait for claude in {worktree:?}"))?;
        Ok(RuntimeOutcome { status, pid })

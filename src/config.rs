...
fn llmc_repo_root() -> Result<PathBuf> {
    ...
    // Try loading settings from current location to find configured target
    let settings = Settings::load(Some(&current_root)).unwrap_or_default();
    let default_target = settings.default_target_path()?;

...
/// Wrap a user prompt with the LLMC preamble.
///
/// Uses the preamble_template from Settings if configured, otherwise uses the
...
pub fn wrap_prompt(
    ...
    let validation_commands = settings.validation_commands_display();

    let preamble = if let Some(template) = settings.preamble_template() {
        // Use custom template with variable substitution
        template
            .replace("{repo_root}", &format!("{repo_root:?}"))
            .replace("{worktree}", &format!("{worktree_path:?}"))
            .replace("{validations}", &validation_commands)
    } else {
        // Use default preamble
        format!(
            "LLMC Preamble\nrepo_root: {repo_root:?}\nworktree: {worktree_path:?}\nFollow AGENTS.md.\nRequired validations: {validation_commands}.\nCreate a git commit with a 5-20 word commit message describing the changes."
        )
    };
...
pub fn process_prompt_pool(pool_path: &Path) -> Result<String> {
    ...
    let Some(start_idx) = selected_prompt_index else {
        return Err(anyhow::anyhow!(
            "No unimplemented prompts found in pool file {pool_path:?}"
        ));
    };

    let end_idx = prompt_starts
        .iter()
        .find(|&&idx| idx > start_idx)
        .copied()
        .unwrap_or(lines.len());
    let prompt_text = lines[(start_idx + 1)..end_idx]
        .join("\n")
        .trim()
        .to_string();

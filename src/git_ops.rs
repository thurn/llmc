...
pub fn ensure_clean_worktree(path: &Path) -> Result<()> {
    ...
    anyhow::ensure!(
        ...
        "Working tree is not clean in {path:?}:\n{status}"
    );
...
pub fn oldest_commit_message(repo_root: &Path, range: &str) -> Result<String> {
    let output = self::git_output(
        repo_root,
        &["log", "--reverse", "--format=%B", "-n", "1", range],
    )?;
...
pub fn amend_commit_message(repo_root: &Path, message: &str) -> Result<()> {
    ...
    let Some(mut stdin) = child.stdin.take() else {
        return Err(anyhow::anyhow!(
            "Failed to open git commit --amend stdin in {repo_root:?}"
        ));
    };
    stdin
    ...
    anyhow::ensure!(
        status.success(),
        "git commit --amend failed in {repo_root:?}"
    );
...
pub fn commit_with_message(repo_root: &Path, message: &str) -> Result<()> {
    ...
    let Some(mut stdin) = child.stdin.take() else {
        return Err(anyhow::anyhow!(
            "Failed to open git commit stdin in {repo_root:?}"
        ));
    };
    stdin
        .write_all(message.as_bytes())
    ...
    let status = child
        .wait()
        .with_context(|| format!("Failed to wait on git commit in {repo_root:?}"))?;
...
pub fn sync_master_to_origin(repo_root: &Path) -> Result<()> {
    ...
    anyhow::ensure!(
        status.success(),
        "git reset --hard origin/master failed in {repo_root:?}"
    );
...
pub fn worktree_add(repo_root: &Path, worktree_path: &Path, branch: &str) -> Result<()> {
    ...
    anyhow::ensure!(
        status.success(),
        "git worktree add failed for {worktree_path:?}"
    );
...
pub fn fetch_from(repo_root: &Path, source: &Path, revision: &str) -> Result<()> {
    ...
    anyhow::ensure!(
        status.success(),
        "git fetch failed for {revision} in {repo_root:?}"
    );
...
pub fn diff_master_agent_difftastic(worktree_path: &Path, branch: &str) -> Result<()> {
    ...
    anyhow::ensure!(
        status.success(),
        "git diff failed for {range} in {worktree_path:?}"
    );
...
pub fn rev_parse(repo_root: &Path, revision: &str) -> Result<String> {
    Ok(self::git_output(repo_root, &["rev-parse", revision])?
        .trim()
        .to_string())
}
...
pub fn diff_worktree_difftastic(worktree_path: &Path) -> Result<()> {
    ...
    anyhow::ensure!(
        status.success(),
        "git diff failed for worktree {worktree_path:?}"
    );
...
pub fn diff_cached_difftastic(worktree_path: &Path) -> Result<()> {
    ...
    anyhow::ensure!(
        status.success(),
        "git diff --cached failed for {worktree_path:?}"
    );
...
pub fn rev_list_count(worktree_path: &Path, range: &str) -> Result<usize> {
    ...
    let count: usize = trimmed
        .parse()
        .with_context(|| format!("Failed to parse rev-list count {trimmed:?}"))?;
...
pub fn remote_origin_url(repo_root: &Path) -> Result<String> {
    Ok(
        self::git_output(repo_root, &["config", "--get", "remote.origin.url"])?
            .trim()
            .to_string(),
    )
}
...
fn git_run(repo_root: &Path, args: &[&str]) -> Result<()> {
    ...
    anyhow::ensure!(
        status.success(),
        "git command failed: git -C {repo_root:?} {args:?}"
    );

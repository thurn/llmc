...
pub fn run(args: &StartArgs, repo_override: Option<&Path>) -> Result<()> {
    ...
    fs::create_dir_all(&paths.llmc_dir).with_context(|| {
        format!(
            "Failed to create llmc dir {llmc_dir:?}",
            llmc_dir = paths.llmc_dir
        )
    })?;
    ...
    anyhow::ensure!(
        ...
        "Worktree already exists: {worktree_path:?}"
    );
    ...
    let settings = Settings::load(Some(&paths.repo_root))?;
    let full_prompt =
        prompt::wrap_prompt(&paths.repo_root, &worktree_path, &user_prompt, &settings);
